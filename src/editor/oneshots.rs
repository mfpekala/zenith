...
fn spawn_standalone_field(
    ...
    points_q: Query<(&EPoint, &IntMoveable, &UIdMarker)>,
    ...
    for eid in points.iter() {
        let Ok((epoint, mv, uid)) = points_q.get(*eid) else {
            warn!("Found non-existent eid when spawning standalone field. Uh oh.");
            return;
        };
        if epoint.kind != EPointKind::Wild {
            warn!("Tried to spawn standalone field while including a non-wild point.");
            return;
        }
        ...
        standalone.field_points.insert(ix, uid.0);
    }
